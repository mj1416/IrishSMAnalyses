...
As a precursor to what follows, %let us assume that the random sample $X_1, ... , X_n$ is independent and identically distributed (i.i.d.). This means that each $X_i$ for $ i = \{1, ... , n\}$ is independent and has the same distribution function (df). Then the order statistics are defined as $X_{1,n} \le X_{2,n} \le , ... , \le X_{n,n}$.
...
\item At each half hour, the maximum from any household is taken. In this way we get a single time series, as in \cite{einmahl16}.
\item A mean excess function for these half hourly maxima were plotted to find a suitable choice of $k$. \todo{put in the image later}This was found to be 130.
...
\begin{figure}
\centering
\includegraphics[scale=0.6]{hh_max_sced.pdf}
\caption{\label{fig:mysced_hh_max} $\hat{c}$ for half hourly max over 503 households for a period of 7 weeks using $k=130$ and $h=0.1$.}
\end{figure}

Both estimations of the scedasis function are reasonably consistent with each other; peaks occur roughly at the same time and as do the troughs. We can delve further and ask when each of the peaks occur to see if there are any patterns. The first and biggest peak occurs between days 597 and 601 which correspond to Friday and the following Tuesday i.e. the peak is occurring over the weekend which comes as no surprise since we saw that weekends have high electric load overall (fig. \ref{fig:sums}). Likewise, the second peak lies between days numbered 617 (Thursday) and 621 (Monday) again occurring in the weekend although maybe as early as Friday. Looking at the third and the smallest of the peaks, we see that it occurs between 625 (Friday) and 629 (Tuesday) meaning again it occurs over the weekend. The final peak, occurring between 633 (Saturday) and 637 (Wednesday), is again seen happening over the weekend as it is closer to the 633 mark. Thus we have evidence that extremes in half hourly maximum are more likely to occur on the weekends. Notably however, we don't a peak at each weekend though it is encouraging that there are more peaks later in the time series as we did see an increase in usage towards the end of the weekly total demand.

Scedasis is not necessarily a measure of where the most extreme event is likely to occur rather an indication of where extremes are more likely to be frequent and it's a somewhat relative scale in that just because one customer (we'll see this later) has high scedasis at one point and not the another doesn't mean that the first customer has higher load but that the first customer is using more than usual.

Further more we can look at a moving average of the half hourly maxima and for large windows, say 100 or 200 half hours, we notice a somewhat similar pattern. \todo{include image later} All the peaks occur at roughly the same time as they do in the estimated scedasis function though the magnitudes are not represented; the first peak in the moving average is smaller than the others and it is the peaks in the middle that are highest. However, for what is rudimentary comparison it is supportive of what we've discussed above.
...
\caption{\label{fig:myintsced_hh_max} $\hat{C}$ for half hourly max over 503 households for a period of 7 weeks with $k=130$.}
\end{figure}

We can continue and consider analogies for the daily losses considered in \cite{einmahl16}; we can consider the positive differences between some aggregate statistic for day $d$ and the same aggregate statistic for day $d-1$ for each household and again aggregate them in some way to have one time-series. However the data set we are currently considering is only 7 weeks long i.e. 49 days so we are limited but it's a valuable start nonetheless.
...
\begin{figure}
\includegraphics[scale=0.8]{pos_diff_sced.pdf}
\caption{\label{fig:pos_diff_sced} Estimated scedasis functions, $\hat{c}$ for various positive differences with $k=3$ (rough scaling), $h=0.1$ and using the biweight kernel.}
\end{figure}
